...
WITHOUT SCALING AND FEEDBACK in PARAMETERS
"""
...
class BasicPowerSGD(Aggregator):
    def __init__(self, params: List[torch.Tensor], config: BasicConfig):
        ...
        self.feedback: List[torch.Tensor] = [torch.zeros_like(p) for p in params]
        self.feedback_per_shape = self._matrices_per_shape(self.feedback)
    ...
    def update_low_rank_weights(self, parameters: List[torch.Tensor], timer=None) -> None:
        ...
        # if self.curr_round == self.synchronization_freq:
        #     self.curr_round = 1
        ...
        for p, mp, f in zip(parameters, self.model_parameters_track, self.feedback):
            # p.mul_((self.synchronization_freq - self.curr_round))
            p.sub_(mp)
            p.add_(f)
            p.mul_(1 / self.curr_round)
        ...
        shape_groups = [
            dict(
                ...
                feedback_per_shape=self.feedback_per_shape[shape],
            )
        ...
        for group in shape_groups:
            for error_gi, feedback_per_shape_gi in zip(
                group["param_batch"], group["feedback_per_shape"]
            ):
                feedback_per_shape_gi.add_(error_gi)
    ...
    def aggregate_parameters(self, parameters: List[torch.Tensor], timer=None) -> List[torch.Tensor]:
        ...
        torch.save(no_compression_weights, f'no_compression_weights_{self.step_counter}.pth')
        ...
        for group in shape_groups:
            for o, pmp, approx in zip(
                ...
                #     print(approx)
                o.copy_(pmp.add_(approx, alpha=self.synchronization_freq))
        ...
        torch.save(output_tensors, f'compressed_weights_{self.step_counter}.pth')
